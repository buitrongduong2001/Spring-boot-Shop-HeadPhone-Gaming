                                ...
                                <div class="card-body">
                                    <div class="table-responsive">
                                        <table id="example" class="table table-bordered" width="100%">
                                            ...
                                            <tbody>
                                            <c:forEach items="${listProductDetail}" var="item" varStatus="loop">
                                                <tr>
                                                    <th scope="row">${loop.count}</th>
                                                    <td>${item.products.title}</td>
                                                    <td>${item.sizes.nameSize}</td>
                                                    <td>${item.quantityStore}</td>


                                                    <td><a type="button" class="btn btn-info"
                                                           href="${pageContext.servletContext.contextPath}/admin/update/product-detail/${item.id}">Edit</a>
                                                    </td>
                                                    <td>
                                                        <button type="button" idd = "${item.id}" onclick="deleteProductDetail($(item.id))"  id="btnDelete" class="btn btn-danger" >Delete</button>
                                                        <!-- Modal -->
